/* $Id: m82510.h,v 1.2 1996/01/16 14:25:09 chris Exp $ */
/*
 * share/m82510.h: definitions for Intel M82510 serial controller
 */
...
typedef union {
     ...
     */
    struct {
	unsigned int	dat;	/* rx data (r) / tx data (w) */
	unsigned int	ger;	/* gen. (i/u) enable (r/w) */
#define bal	dat		/* BRGA low (when DLAB=1) */
#define bah	ger		/* BRGA high */
	unsigned int	gir;	/* gen. i/u (r) / bank select (w) */
#define bank	nas.gir
	unsigned int	lcr;	/* line control */
	unsigned int	mcr;	/* modem control (r/w) */
	unsigned int	lsr;	/* line status (r/w) */
	unsigned int	msr;	/* modem status (r/w) */
	unsigned int	acr0;	/* addr/ctrl char #0 */
    } nas;
    ...
    struct {
	unsigned int	dat;		/* rx data (r) / tx data (w) */
	unsigned int	flg;		/* rx flags (r) / tx flags (w) */
	unsigned int	gir;		/* gen. i/u (r) / bank select (w) */
	unsigned int	tmst_cr;	/* timr stat (r) / timr ctrl (w) */
	unsigned int	flr_mcr;	/* fifo level (r) / mdm ctrl (w) */
	unsigned int	rst_rcm;	/* rx stat (r) / rx cmd (w) */
	unsigned int	msr_tcm;	/* mdm stat (r) / tx cmd (w) */
	unsigned int	gsr_icm;	/* gen. stat (r) / int. cmd (w) */
    } wrk;
     ...
     */
    struct {
	unsigned int	xxx;		/* unused */
	unsigned int	fmd;		/* fifo mode (r/w) */
	unsigned int	gir;		/* gen. i/u (r) / bank select (w) */
	unsigned int	tmd;		/* tx mode (r/w) */
	unsigned int	imd;		/* internal mode (r/w) */ 
	unsigned int	acr1;		/* addr/ctrl char #1 */
	unsigned int	rie;		/* rx i/u enable (r/w) */
	unsigned int	rmd;		/* rx mode (r/w) */
    } gcfg;
     ...
     */
    struct {
	unsigned int	clcf;		/* clock config (r/w) */
	unsigned int	bacf;		/* BRGA config (r/w) */
#define bbl	clcf			/* BRGB low (when DLAB=1) */
#define bbh	bacf			/* BRGB high */
	unsigned int	gir;		/* gen. i/u (r) / bank select (w) */
	unsigned int	bbcf;		/* BRGB config (r/w) */
	unsigned int	pmd;		/* i/o pin mode (r/w) */
	unsigned int	mie;		/* modem i/u enable (r/w) */
	unsigned int	tmie;		/* timer i/u enable (r/w) */
	unsigned int	xxx;		/* unused */
    } mcfg;
