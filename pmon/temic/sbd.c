/*
 * temic/sbd.c: board support for Algorithmics/Temic module
	      ...
	      int (*set)(const char *, const char *))
{
    ...
    if (!(*get) ("ethaddr"))
	(*set) ("ethaddr", "00:40:bc:00:30:00");

    /* default to using dynamic bootp for internet address */
    if (!(*get) ("bootp"))
	(*set) ("bootp", "pri");
#endif
    ...
    char *exc;
{
    ...
    /* read exception to PROM, encoding data in address */
    register volatile unsigned char *prom = (void *) 0xbf7fe00;
#define pByte(x) (void)prom[(x) & 0xff]
