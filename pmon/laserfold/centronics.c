/* $Id: centronics.c,v 1.1 1996/12/10 11:58:10 nigel Exp $ */
#include <mips.h>
#include <termio.h>
#include <pmon.h>
#include <sbd.h>
...
ceninit (void)
{
    ...
    /* select parallel port 1 (~port2) */
    pio_bis (PIO_PP_NSEL2);
    pio_bic (PIO_PP_NSEL1);
...
cenreset (int chan)
{
	...
	/* select parallel port 2 (~port1) */
	pio_bis (PIO_PP_NSEL1);
	pio_bic (PIO_PP_NSEL2);
    }
    cenrxstop = 0;
    cenflush ();
    ...
    /* remove external BUSY signal */
    G10_REG(G10_PIOOUT) &= ~G10_PIO_CENBUSY;
    return 0;
}
