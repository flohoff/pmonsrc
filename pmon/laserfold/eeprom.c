/* $Id: eeprom.c,v 1.1 1996/12/10 11:58:11 nigel Exp $ */
/*
 * eeprom.c: routines to handle serial EEPROM on Libra
 */
...
/* 
 * AT93C66 has 256x16-bits -> 8 address bits 
 */
#define ABITS		8
...
/*
 * EEROM accessed thru parallel i/o.
 */
#include <pio.h>
...
do { \
	WAIT (); if (on) pio_bis (bit); else pio_bic (bit); \
} while (0);


/* chip select */
static	void CS (int on)	{TOGGLE (PIO_E2_CS, on)}

/* shift clock */
static	void SK (int on)	{TOGGLE (PIO_E2_SK, on)}

/* my data out, chip's data in */
static	void DOUT (int on)	{TOGGLE (PIO_E2_DIN, on)}

/* my data in, chip's data out */
static inline unsigned int DIN (void)	
{ 
    WAIT (); 
    return (pio_get (PIO_E2_DOUT) ? 1 : 0);
}
