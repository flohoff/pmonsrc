...
#ifdef IN_PMON
#define _mips_intdisable() splhigh()
#define _mips_intrestore(x) splx(x)
#endif
...
#ifndef FROM_BOOTSECSIZE
/* chip-specific boot sector size */
#define FROM_BOOTSECSIZE(fcp, size) (size)
#endif
...
static const struct fromdevinfo devinfo[] = {
    ...
    {"29DL800T", MAN_AMD,	0x4a, FBTOP,	0x100000, 0x10000, TYPE_aF400},
    {"29DL800B", MAN_AMD,	0xcb, FBTOP,	0x100000, 0x10000, TYPE_aF400},
#endif
    ...
    {"28F160T",	 MAN_SHARP,	0x48, FBTOP,	0x200000, 0x10000, TYPE_sF160},
    {"28F160B",	 MAN_SHARP,	0x49, FBBOT,	0x200000, 0x10000, TYPE_sF160},

    {"28F800B",	 MAN_SHARP,	0x4b, FBBOT,	0x800000, 0x10000, TYPE_sF160},
#endif
    ...
    {"29LV650",	MAN_AMD,	0xd7, FBNONE,	0x800000, 0x10000, TYPE_LV600, 0, 0x10},
    {"29LV651",	MAN_AMD,	0xd7, FBNONE,	0x800000, 0x10000, TYPE_LV600, 0, 0x00},

    /* AMD 29DL devices (8/16 bit, asymmetric banks) */

    {"29DL161T",MAN_AMD,	0x36, FBTOP,	0x200000, 0x10000, TYPE_aDL,
     0x10000},
    {"29DL161B",MAN_AMD,	0x39, FBBOT,	0x200000, 0x10000, TYPE_aDL,
     0x10000},
    {"29DL162T",MAN_AMD,	0x2d, FBTOP,	0x200000, 0x10000, TYPE_aDL,
     0x40000},
    {"29DL162B",MAN_AMD,	0x2e, FBBOT,	0x200000, 0x10000, TYPE_aDL,
     0x40000},
    {"29DL163T",MAN_AMD,	0x28, FBTOP,	0x200000, 0x10000, TYPE_aDL,
     0x80000},
    {"29DL163B",MAN_AMD,	0x2b, FBBOT,	0x200000, 0x10000, TYPE_aDL,
     0x80000},
    {"29DL164T",MAN_AMD,	0x33, FBTOP,	0x200000, 0x10000, TYPE_aDL,
     0x100000},
    {"29DL164B",MAN_AMD,	0x35, FBBOT,	0x200000, 0x10000, TYPE_aDL,
     0x100000},

    {"29DL321T",MAN_AMD,	0x59, FBTOP,	0x400000, 0x10000, TYPE_aDL,
     0x10000},
    {"29DL321B",MAN_AMD,	0x5a, FBBOT,	0x400000, 0x10000, TYPE_aDL,
     0x10000},
    ...
    {"29DL323B",MAN_AMD,	0x53, FBBOT,	0x400000, 0x10000, TYPE_aDL,
     0x100000},
    {"29DL324T",MAN_AMD,	0x5c, FBTOP,	0x400000, 0x10000, TYPE_aDL,
     0x200000},
    {"29DL324B",MAN_AMD,	0x5f, FBBOT,	0x400000, 0x10000, TYPE_aDL,
     0x200000},
...
itmalloc (size_t size)
{
    ...
    DBG ("FLASH malloc: requesting %d words\n", size);
...
#define frompoke(addr, pval, flags) \
	...
	(void) *(volatile int *) KVA0_TO_KVA1 (&_flush); \
     } while (0)
#endif
...
static INLINE void
fromcmd_amdoffs (fromcookie_t fcp, unsigned int bank, unsigned int col, 
		 unsigned int flags, fromdev_t cmd, int unlock_again,
		 unsigned int offs)
{
    from_pt cmd3 = fromaddr (fcp, bank, offs | fcp->cmd1_offs, col, 
			     flags | _FLASHDEV_CMD);
#if FROM_FAMILY_AMD
...
#endif
    frompoke (cmd3, fromgroup (cmd), FCMD);
	
    if ((fcp->flags & FROMFLG_DELAY) && cmd != FROMCMD_AMD_PROGRAM) {
	/* Some Atmel parts need a 10ms delay */
#if defined(mips_getcount) && defined(_mips_mfc0) && !__mips16
	/* cache/uncached independent */
	unsigned int compare = mips_getcount () + CACHEMS (11);
	while ((int)(compare - mips_getcount ()) > 0)
	    continue;
#else
	/* cache/uncached dependent */
...
fromcmd_amd (fromcookie_t fcp, unsigned int bank, unsigned int col, 
	     unsigned int flags, fromdev_t cmd, int unlock_again)
{
    fromcmd_amdoffs (fcp, bank, col, flags, cmd, unlock_again, 0);
}
	    ...
	    (fcp, offs, manp, devp, xidp))
{
	...
	unsigned int offs = addr & ~(((fcp->secsize / FROM_WIDTH) - 1) >> iashift);
	fromcmd_amdoffs (fcp, bank, col, FCMD, FROMCMD_AUTOSELECT, 0, offs);
    }
    else
    ...
    xp = fromaddr (fcp, bank, 3 << iashift, col, FCMD);
    man = frompeek (mp, FCMD);
    dev = frompeek (dp, FCMD);
    xid = frompeek (xp, FCMD);

    if (manp)
	DBG ("flash probe offs=%x: manuf@%x=%x devid@%x=%x xid@%x=%x\n",
		 offs, mp, man, dp, dev, xp, xid);
	    ...
	    unsigned int *soffsp, unsigned int *ssizep)
{
	...
	soffs = banksize;
	secno = nsec;
	while ((ssize = (FROM_BOOTSECSIZE (fcp, *bootsecs++) * FROM_NCOLS))) {
	    ...
	    unsigned int *soffsp, unsigned int *ssizep)
{
	...
	for (soffs = i = 0; soffs < fcp->size; soffs += ssize, i++) {
	    ...
	    /* Atmel boot block protection on top and bottom 16K only */
	    if (fcp->flags & FROMFLG_ATMEL) {
		if (soffs >= 0x4000 * FROM_LINESZ
		    || soffs < fcp->size - 0x4000 * FROM_LINESZ) {
		    /* middle sectors cannot be protected */
		    fcp->protect[i] = 0;
		    continue;
		}
		if (soffs != 0 && soffs !=  fcp->size - 0x4000 * FROM_LINESZ) {
		    /* copy last protect status */
		    fcp->protect[i] = prot;
		    continue;
		}
	    }
	       ...
	       void *buf, unsigned int nb, unsigned int flags)
{
	...
	memcpy (mem, (void *)(fcp->rbase + fcp->virtoffs + offs), nb);
	return FLASHDEV_OK;
    }
	...
	if (offs & FROM_LINEMASK) {
	    ...
	    from_pt rbase = fcp->rbase + fcp->virtoffs;
...
_flashrom_probe (struct fromcookie *fcp, unsigned int offs)
{
    ...
    (void) fromautoselect (fcp, offs, &man, &dev, &xid);
#endif
	...
	if ((dv->man == man || (dv->man == MAN_AMD && man == MAN_FUJITSU))
	    && dv->dev == dev
	    && ((dv->flags & FROMFLG_AUTOEX) ? (xid == dv->xid) : 1))
	    /* found it */
	    break;
    }
