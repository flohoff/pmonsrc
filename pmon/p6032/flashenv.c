...
env_reset (void)
{
    ...
    env_flush ();
    for (size = 0; size < env_maxsize; size += info.ssize) {
	/* XXX assumes [env_offs,env_maxsize] are exactly sector aligned */
	(void) flash_info (env_cookie, env_offs + size, &info);
	(void) flash_erasesector (env_cookie, env_offs + size);
    }
...
env_invalid (void)
{
    ...
    /* default to missing flash */
    env_state = env_init_state = ENVSTATE_MISSING;

    /* find out how much space is available to us */
    env_cookie = _sbd_flashenv_open ();
    if (env_cookie == FLASH_MISSING || env_cookie == FLASH_UNKNOWN)
	return env_state;

    /* get partition table */
    if (flash_parts (env_cookie, &parts) != FLASHDEV_OK)
	return env_state;
    
    /* look for environment partition */
    for (i = 0; i < FLASHNPART; i++) {
	if (parts.part[i].type == FLASHPART_ENV) {
	    env_offs = parts.part[i].offs;
	    env_maxsize = parts.part[i].size;
	    break;
	}
    }

    /* have we got enough/any space? */
    if (i >= FLASHNPART || env_maxsize < ENVOFF_BASE + 64)
	return env_state;
...
env_find (const char *name)
{
    ...
    if (name == 0 || (namelen = strlen (name)) >= 254)
	return (0);
...
#else
    unsigned char *cbuf = malloc (env_maxsize);
#endif
...
#if 0
    /* set free part to 0xff */
    memset (cp, 0xff, envfree);
    env_setbytes (ENVOFF_BASE, cbuf, envsize + envfree);
#else
    /* assume env_reset has erased sector containing environment */
