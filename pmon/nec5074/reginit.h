...
#define WR_INIT(mod,addr,val) \
	.word	OP_WR|MOD_/**/mod,PA_TO_KVA1(addr);\
	rword	(val),0
	
#define RD_INIT(mod,addr) \
	.word	OP_RD|MOD_/**/mod,PA_TO_KVA1(addr);\
	rword	0,0
	
#define RMW_INIT(mod,addr,and,or) \
	.word	OP_RMW|MOD_/**/mod,PA_TO_KVA1(addr);\
	rword	(and),(or)
	
#define WAIT_INIT(mod,addr,and,or) \
	.word	OP_WAIT|MOD_/**/mod,PA_TO_KVA1(addr);\
	rword	(mask),(val)
...
#define WR_INIT(mod,addr,val) \
	{OP_WR | MOD_ ## mod, PA_TO_KVA1(addr), (val)}
	
#define RD_INIT(mod,addr) \
	{OP_RD | MOD_ ## mod, PA_TO_KVA1(addr)}
	
#define RMW_INIT(mod,addr,and,or) \
	{OP_RMW | MOD_ ## mod, PA_TO_KVA1(addr), (and), (or)}
	
#define WAIT_INIT(mod,addr,and,or) \
	{OP_WAIT | MOD_ ## mod, PA_TO_KVA1(addr), (mask), (val)}
