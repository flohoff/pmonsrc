...
do { \
	WAIT (); if (on) zpiob_bis (bit); else zpiob_bic (bit); \
} while (0);


/* chip select */
static inline void CS (int on)	{TOGGLE (ZPIOB_E2_CS, on)}

/* shift clock */
static inline void SK (int on)	{TOGGLE (ZPIOB_E2_SK, on)}

/* my data out, chip's data in */
static inline void DOUT (int on) {TOGGLE (ZPIOB_E2_DI, on)}

/* my data in, chip's data out */
static inline unsigned int DIN (void)	
{ 
    WAIT (); 
    return (zpiob_get () & ZPIOB_E2_DO) ? 1 : 0;
}
