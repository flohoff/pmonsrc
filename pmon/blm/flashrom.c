 ...
 *
 * Not supported yet:
 *	AMD 29DL range (non-uniform banks)
...
#ifndef fromoffs
#define fromoffs(fcp, bank, addr, flags) 				\
  (((bank) << FROM_BANKSIZE_LOG2(fcp)) + ((addr) * FROM_LINESZ))
#endif
	    ...
	    unsigned int *bankp, unsigned int *colp)
{
    ...
    *bankp = offs >> banksize_log2;
    addr = (offs & ((1 << banksize_log2) - 1)) / FROM_WIDTH;
    if (colp) {
	    ...
	    (fcp, offs, manp, devp))
{
	...
	from_pt gsr;
	frompoke (mp, fromgroup (FROMCMD_IBCS_RESET));
	frompoke (mp, fromgroup (FROMCMD_SHARP_UPSTAT));
	frompoke (mp, fromgroup (FROMCMD_IBCS_CONFIRM));
	frompoke (mp, fromgroup (FROMCMD_SHARP_RXSR));
	gsr = fromaddr (fcp, bank, 2 << iashift, col, FCMD);
	while ((frompeek (gsr) & fromgroup (SGSR_READY))
	    ...
	    (cp))
{
	...
	for (bank = 0; bank < FROM_NBANKS(fcp); bank++) {
	    ...
	    foreach_group (col) {
		int stat;
		stat = frompoll_amd (fcp, fp + frompcol (col, FSECT), ~0);
		if (stat != FLASHDEV_OK)
		    status = stat;
	    }
	}
	    ...
	    (cp, secoffset))
{
	...
	if (fcp->flags & FROMFLG_IBCS) {
		...
		frompoke (cp, fromgroup (FROMCMD_IBCS_ERASE));
		frompoke (cp, fromgroup (FROMCMD_IBCS_CONFIRM));
	    }
	}
	else if (fcp->flags & FROMFLG_ATMEL) {
		...
		frompoke (cp, fromgroup (FROMCMD_AMD_ERASESECT));
	    }
	}
	...
	foreach_group (col) {
	    ...
	    int stat;
	    if (fcp->flags & FROMFLG_IBCS)
		stat = frompoll_ibcs (fcp, cp, ~0);
	    else if (fcp->flags & (FROMFLG_AMD | FROMFLG_ATMEL))
		stat = frompoll_amd (fcp, cp, ~0);
	    else
		stat = FLASHDEV_FAIL;
	    if (stat != FLASHDEV_OK)
		status = stat;
	    ...
	    (fcp, v, nv, flags))
{
	...
	if (fcp->flags & FROMFLG_ATMEL) {
		...
		foreach_group (col) {
		    ...
		    if (frompeek (cp) != nv) {
			if (fcp->flags & FROMFLG_IBCS)
			    frompoke (cp, fromgroup (FROMCMD_IBCS_PROGRAM));
			else if (fcp->flags & FROMFLG_AMD)
		...
		foreach_group (col) {
			...
			fromgroup_t nv = mp->v;
			if (fcp->flags & FROMFLG_IBCS)
			    stat = frompoll_ibcs (fcp, cp, nv);
			else if (fcp->flags & FROMFLG_AMD)
			    stat = frompoll_amd (fcp, cp, nv);
			else
			    stat = FLASHDEV_FAIL;
			if (stat != FLASHDEV_OK)
			    status = stat;
