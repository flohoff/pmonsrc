/* $Id: if_xdssonic.c,v 1.5 1998/12/28 14:13:00 nigel Exp $ */
/*
...
static void copy_data_to_host (caddr_t sp, caddr_t dp, int len);
static void copy_host_to_data (caddr_t sp, caddr_t dp, int len);
static void xds_compress_buffer (caddr_t addr, int len);
static void xds_expand_buffer (caddr_t addr, int len);

static void sonictxdump (char *msg, struct TXpkt *txp);
static void sonicdump (char *msg, volatile struct sn_reg *csr);
...
#ifdef XDSSONICBUG
#define UPPER(x)	(K0_TO_PHYS((x)) >> 17)
#define LOWER(x)	((((unsigned int)(x)) >> 1) & 0xffff)
#else
...
#endif
    log (LOG_DEBUG, "eninit completed\n");
...
snstartup (struct sn_softc *sn)
{
    ...
    *(u_int *) PHYS_TO_K1 (BCRR) |= BCRR_ETH; wbflush();
    ...
    /* config it */
#if defined(XDS)
    csr->s_dcr = DCR_ASYNC|DCR_WAIT1|DCR_DW32|DCR_RFT24|DCR_TFT16;
#elif defined(P4000)
    csr->s_dcr = 
      DCR_EXBUS|DCR_LBR|
...
#undef XDSLOWMEM
...
#if defined(XDS)
#ifdef XDSLOWMEM
    rba = (char *)PHYS_TO_K1(0x07050000);
#else
    rba = (char *)PHYS_TO_K1(SRAM_NET);
#endif
    ...
    /* check sanity of buffer addreseses */
...
snwatch (int unit)
{
	...
	log (LOG_WARNING, "%s%d: Tx - %s\n",
	     sn->sn_if.if_name, sn->sn_if.if_unit,
	     (MASK(mtdhead->mtd_txp->status) == 0) ? "timeout" : "lost interrupt");
	/* snrestart (sn); */
        snclosedown (sn);
    }
    (void) splx (s);
...
static void
xds_compress_buffer (caddr_t addr, int len)
{
    int words = (len + 3) >> 2;
    unsigned int *sp, *dp;
    /* ASSERT(IS_K1SEG(addr)); */

/* printf ("xdr_compress_buffer (0x%08x, 0x%x)\n", addr, len); */
...
static void
xds_expand_buffer (caddr_t addr, int len)
{
    int words = (len + 3) >> 2;
    unsigned int *sp, *dp;
    /* ASSERT(IS_K1SEG(addr)); */
/* printf ("xds_expand_buffer (0x%08x, %d)\n", addr, len); */
...
#ifdef XDS
static void
copy_host_to_data (caddr_t sp, caddr_t dp, int len)
{
    unsigned int *wp;
    unsigned int w;
    ...
    soffset = (unsigned int)sp & 3;
    doffset = (unsigned int)dp & 3;

    if (doffset) {
	/* first align dp */
	wp = (unsigned int *)PHYS_TO_K1((unsigned int)dp & ~3);
	w = *wp;
	for (shift = 24 - 8*doffset; shift >= 0; shift -= 8) {
	    w = (w & ~(0xff << shift)) | (*(unsigned char *)sp << shift);
	    dp++;
	    sp++;
	    len--;
	    if (len <= 0) {
	...
	*wp = w;
	...
	/* read 4 bytes, write a word */
	while (len >= 4) {
	    w =  *(unsigned char *)(sp+0) << 24;
	    w |= *(unsigned char *)(sp+1) << 16;
	    w |= *(unsigned char *)(sp+2) <<  8;
	    w |= *(unsigned char *)(sp+3);
	    *(unsigned int *)dp = w;
	    sp += 4;
	    dp += 4;
	    len -= 4;
...
static void
copy_data_to_host (caddr_t sp, caddr_t dp, int len)
{
    unsigned int *wp, w;
    unsigned int soffset, doffset;
    int shift;
    ...
    if (soffset) {
	/* first align sp */
	w = *(unsigned int *)((unsigned int)sp & ~3);
	for (shift = 24 - 8*soffset; shift >= 0; shift -= 8) {
	    *dp = w >> shift;
	    dp++;
	    sp++;
	    len--;
	...
	/* read a word, write 4 bytes */
	while (len >= 4) {
	    w = *(unsigned int *)sp;
	    *(unsigned char *)(dp+0) = w >> 24;
	    *(unsigned char *)(dp+1) = w >> 16;
	    *(unsigned char *)(dp+2) = w >> 8;
	    *(unsigned char *)(dp+3) = w;
	    sp += 4;
	    dp += 4;
	    len -= 4;
	}
