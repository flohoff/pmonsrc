/* $Id: whetd.c,v 1.2 1996/01/16 14:17:09 chris Exp $ */
...
prprof();
#endif
	printf("time=%d secs, %d usecs,",time1-time0,t1-t0);
	if (time1-time0 <= 0) exit(1);
	printf (" Double Whetstone KIPS %d\n",(int)((100*LOOP*II)/(time1-time0)));
	exit(0);
}	/* END; */

PA(E) double E[5]; { /* SUBROUTINE PA(E); */
/*	IMPLICIT REAL*8 (A-H,O-Z); */
/*	DIMENSION E(4); */
...
P3(X,Y,Z) double *X,*Y,*Z; { /* SUBROUTINE P3(X,Y,Z); */
/*	IMPLICIT REAL*8 (A-H,O-Z); */
/*	COMMON T,T1,T2,E1(4),J,K,L; */
	double Y1;
	X1 = *X;
...
POUT(N,J,K,X1,X2,X3,X4) int N,J,K; double X1,X2,X3,X4; { /* SUBROUTINE POUT(N,J,K,X1,X2,X3,X4); */
/*	IMPLICIT REAL*8 (A-H,O-Z); */
